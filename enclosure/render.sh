#!/usr/bin/env bash
# Regenerates every STL and preview PNG from forager_enclosure.scad.
# Run after any change to the .scad -- the checked-in artifacts are outputs.
set -euo pipefail
cd "$(dirname "$0")"

SCAD=forager_enclosure.scad
...
openscad -D 'part="tray"'  $IMG --camera=0,0,0,0,180,0,0   -o preview/04_tray_exterior.png     "$SCAD"
openscad -D 'part="both"'  $IMG --camera=0,0,0,55,0,25,0   -o preview/05_assembled_overview.png "$SCAD"
